...
const generateEmailHTML =
  require("../templates/emailTemplate").generateEmailHTML;
...
const getSingleUser = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    ...
    stripeAccountId: user.stripeAccountId ? user.stripeAccountId : null,
...
const requestForCreator = asyncHandler(async (req, res) => {
  ...
  const userUpdated = await User.findByIdAndUpdate(
    req.params.id,
    { creator, attachmentsForCreator, creatorVerificationStatus: "PENDING" },
    {
      new: true,
    }
  );
...
const creatorVerificationApproved = asyncHandler(async (req, res) => {
  const userVerified = await User.findOne({
    creator: req.params.verificationToken,
    creatorVerificationStatus: "VERIFIED",
  });

  if (userVerified) {
    res.status(400).json({
      message: `${userVerified.name} is already verified and a creator`,
    });
    return;
  }
  ...
  if (!userVerified && req.params.status === "rejected") {
    ...
    res
      .status(200)
      .json({ message: `${user.name} request for creator has been rejected` });
    return;
  }
  ...
  if (!userVerified && req.params.status === "verified") {
    const user = await User.findOneAndUpdate(
      { creator: req.params.verificationToken },
      { creatorVerificationStatus: "VERIFIED", role: "creator" },
      {
        new: true,
      }
    );
    ...
    res.status(200).json({
      message: `${user.name} has been successfully verified and becomes a creator`,
    });
    return;
  }
